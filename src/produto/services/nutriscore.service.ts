...
export class NutriScoreService {
  ...
  private async chamarGeminiAPI(prompt: string): Promise<string> {
    ...
    if (!API_KEY) {
      throw new HttpException('API Gemini não configurada', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  private obterDadosNutricionais(resposta: string): NutriScoreData {
    const extrairValor = (padrao: RegExp): number => {
      ...
      const valores = match[1].split('-').map(v => 
        parseFloat(v.trim().replace(',', '.').replace(/[^\d.]/g, ''))
      );
    ...
    return {
      valorEnergetico: extrairValor(/Valor energético.*?(\d+(?:,\d+)?(?:\s*-\s*\d+(?:,\d+)?)?)\s*kcal/i),
      acucaresTotais: extrairValor(/Açúcares totais.*?(\d+(?:,\d+)?(?:\s*-\s*\d+(?:,\d+)?)?)\s*g/i),
      gordurasSaturadas: extrairValor(/Gorduras saturadas.*?(\d+(?:,\d+)?(?:\s*-\s*\d+(?:,\d+)?)?)\s*g/i),
      sodio: extrairValor(/Sódio.*?(\d+(?:,\d+)?(?:\s*-\s*\d+(?:,\d+)?)?)\s*mg/i),
      proteinas: extrairValor(/Proteínas.*?(\d+(?:,\d+)?(?:\s*-\s*\d+(?:,\d+)?)?)\s*g/i),
      fibrasAlimentares: extrairValor(/Fibras alimentares.*?(\d+(?:,\d+)?(?:\s*-\s*\d+(?:,\d+)?)?)\s*g/i),
      percentualFrutasLegumesOleaginosas: extrairValor(/% de frutas, legumes e oleaginosas.*?(\d+(?:,\d+)?(?:\s*-\s*\d+(?:,\d+)?)?)\s*%/i),
    };
  }
